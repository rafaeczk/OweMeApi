...
public class AddFriendByCodeHandler(AppDbContext context, FriendsService service) : IRequestHandler<AddFriendByCodeCommand, Result<AddFriendResponseDTO>>
{
    public async Task<Result<AddFriendResponseDTO>> Handle(AddFriendByCodeCommand request, CancellationToken ct)
    {
        ...
        if (friendCode == null || friendCode.ExpiresAt < DateTime.UtcNow)
            return Result.Failure("Code not found or expired", ErrorCode.NotFound);

        if (friendCode.UserId == request.UserId)
            return Result.Failure("You cannot enter your own code", ErrorCode.BadRequest);
